...
var doToggle = function doToggle(transition, video, meeting) {
  _media.default.setLocalTrack(!video.mute, meeting.mediaProperties.videoTrack);

  var meetingAudio = meeting.audio;
  var audioMuted = meetingAudio ? meetingAudio.muted : true;
  return _util.default.remoteUpdateAudioVideo(audioMuted, video.mute, meeting).then(function (locus) {
    _loggerProxy.default.logger.log("Meeting:video#doToggle --> VideoStateMachine->onAfterToggle#".concat(transition.event, " fired! State changed from '").concat(transition.from, "' to '").concat(transition.to, "' with transition '").concat(transition.transition, "''."));

    return locus;
  }).catch(function (remoteUpdateError) {
    _loggerProxy.default.logger.log("Meeting:video#doToggle --> VideoStateMachine->onBeforeToggle#".concat(transition.event, " fired! State failed to change with transition '").concat(transition.transition, "''. After local Video toggle failed, resetting remote also failed, meeting video in bad state with error: ").concat(remoteUpdateError, "."));
      ...
      methods: {
        ...
        onBeforeToggle: function onBeforeToggle(transition, video) {
          ...
          if (transition.from !== transition.to) {
            return doToggle(transition, video, meeting).then(function (locus) {
              _this.setData(video);
         ...
         * @returns {Object} this.data which contains {muted, self}
         */
        onAfterToggle: function onAfterToggle() {
          return _promise.default.resolve(this.data);
        },
...
var _default = VideoStateMachine;
